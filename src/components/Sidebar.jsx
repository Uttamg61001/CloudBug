...
import { FaAngleDoubleLeft, FaAngleDoubleRight, FaBell, FaCaretDown } from 'react-icons/fa';
...
const FaAngleDoubleLeftIcon = styled(FaAngleDoubleLeft, { shouldForwardProp: (prop) => true })(
    ({ theme, open }) => ({
        zIndex: theme.zIndex.drawer + 1,
        position:'absolute'
    }),
);
const FaAngleDoubleRightIcon = styled(FaAngleDoubleRight, { shouldForwardProp: (prop) => true })(
    ({ theme, open }) => ({
        zIndex: theme.zIndex.drawer + 1,
        position:'absolute'
    }),
...
const Sidebar = () => {
    ...
    return (
        <div className='relative h-full border-none shadow-lg' >
            ...
            <Drawer variant="permanent" open={open} className='relative'>
                ...
                </DrawerHeader>
                {
                    open ?
                        ...
                        : 
                        <div className="addservicebtn flex justify-center my-8">
                            <Link to="/select-service" className=" bg-[#51459E] text-white px-3 py-3 rounded-md shadow-lg"><IoMdAdd size={25} /></Link>
                        </div>
                }
                {open ?
                    ...
                    <div className="navlist mt-4 flex flex-col justify-center ">
                        {navItems?.map((item, index) => {
                            return (
                                <Link to={item.link} key={index} className='flex flex-col justify-between gap-2 items-center my-2' >
                                    <div className={`navitem flex items-center justify-center p-4 ${window.location.pathname == item.link ? "bg-[#51459E]" : "bg-[#ECEFF7] "}  rounded-lg shadow-md`}>
                                        <img src={item?.icon} color='black' alt="" />
                                    </div>
                                </Link>
            ...
            {
                        open ?
                            <FaAngleDoubleLeftIcon onClick={handleDrawerClose} size={35} className={` absolute -right-3 top-28 p-2 shadow-lg cursor-pointer rounded-full bg-slate-200 `} />
                            :
                            <FaAngleDoubleRightIcon onClick={handleDrawerOpen} size={35} className={` absolute -right-3 top-28 p-2 shadow-lg cursor-pointer rounded-full bg-slate-200 `} />
                    }
        </div>
    )
}
